...
# Replace your current Firebase initialization with:
try:
...
def monitor_breakdowns():
    """Detects breakdowns by checking movement history (runs in parallel)"""
    while True:
        try:
            with app.app_context():
                bus = db.session.get(Bus, "101")
                ...
                # --- Breakdown Condition 1: No movement >100m in 15 mins ---
                ...
                if len(last_3_updates) >= 3:
                    # Calculate distance between oldest and newest update
                    ...
                    distance_moved = haversine_distance(lat1, lon1, lat2, lon2) * 1000  # Convert km to meters
                    is_stationary = distance_moved < 100  # Threshold: 100 meters

                # --- Breakdown Condition 2: No GPS updates for >20 mins ---
                latest_gps = BusGPS.query.filter_by(bus_id="101").order_by(BusGPS.id.desc()).first()
                no_recent_data = latest_gps and (datetime.now() - latest_gps.timestamp > timedelta(minutes=20))

                # --- Update Firebase breakdown flag ---
...
# New breakdown monitoring thread
breakdown_thread = threading.Thread(target=monitor_breakdowns)
breakdown_thread.daemon = True
...
@app.route('/bus/<bus_number>/visualization')
def bus_visualization(bus_number):
    try:
        ...
        if not from_stop or not to_stop:
            abort(400, description="Missing start/end parameters")
        ...
        current_index = next(
            (i for i, rs in enumerate(route_stops) 
             if bus.current_stop and rs.stop.name == bus.current_stop.name), 0)
        
        # Calculate progress (0-100)
        progress = 0
        if route_stops:
            progress = min(100, max(0, (current_index / (len(route_stops) - 1)) * 100))
        
        # Get latest GPS data for speed
        latest_gps = BusGPS.query.filter_by(bus_id=bus.id).order_by(BusGPS.id.desc()).first()
        speed = latest_gps.speed if latest_gps and latest_gps.speed else 30
        
        # Calculate ETAs for each stop using the same method as calculate_arrival_time
        now = datetime.now()
        stop_positions = []
        ...
        for i, rs in enumerate(route_stops):
            if i < current_index:
                ...
                eta_time = "Current location"
            else:
                # Calculate distance-based time if possible
                if speed > 0 and bus.current_stop:
                    total_distance = 0
                    prev_lat = bus.current_stop.latitude
                    prev_lng = bus.current_stop.longitude
                    
                    for j in range(current_index + 1, i + 1):
                        next_stop = route_stops[j].stop
                        total_distance += haversine_distance(prev_lat, prev_lng, 
                    ...
                    # Fall back to static time_to_next values
                    time_minutes = sum(rs.stop.time_to_next for rs in route_stops[current_index:i])
                
                eta_time = (now + timedelta(minutes=time_minutes)).strftime("%I:%M %p")
            
            stop_positions.append({
                'name': rs.stop.name,
                'position': (i / (len(route_stops) - 1)) * 100 if len(route_stops) > 1 else 0,
                'is_current': rs.stop.name == bus.current_stop.name if bus.current_stop else False,
                'is_from': rs.stop.name == from_stop,
                'is_to': rs.stop.name == to_stop,
                'eta': eta_time if i >= current_index else None
            })
        
        return render_template(
            "bus-visualization.html",
            bus=bus,
            from_stop=from_stop,
            to_stop=to_stop,
            progress=progress,
...
@app.route('/debug')
def debug():
    return str(app.jinja_loader.list_templates())
